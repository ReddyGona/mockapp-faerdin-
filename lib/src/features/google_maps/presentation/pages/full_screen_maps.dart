...
class _GoogleMapsFullScreenState extends State<GoogleMapsFullScreen> {
  ...
  void moveCameraToFitBounds() {
    ...
    final List<LatLng> allCoordinates = [
      googlePlex,
      destinationPlex,
      ...polylines.values.expand((polyline) => polyline.points)
    ];

    final LatLngBounds bounds = LatLngBounds(
      southwest: LatLng(
        allCoordinates.map((coord) => coord.latitude).reduce(min),
        allCoordinates.map((coord) => coord.longitude).reduce(min),
      ),
      northeast: LatLng(
        allCoordinates.map((coord) => coord.latitude).reduce(max),
        allCoordinates.map((coord) => coord.longitude).reduce(max),
      ),
    );
